...
while not game_over_flag:
    ...
    for coin in collided_coins:
        ...
        if COINS_COLLECTED % 10 == 0: 
         ...
         new_coin = tenge()
         tenges.add(new_coin)
         all.sprites.add(new_coin)
         new_coin.rect.top = 0
         new_coin.rect.center = (random.randint(40,SCREEN_WIDTH -40),0)
    tenge_coins_collided = pygame.sprite.spritecollide(P1,tenges,True)
    for tenge_coin in tenge_coins_collided:
        COINS_COLLECTED +=5
        pygame.mixer.Sound('coin.wav').play()
        tenge_coin.kill()
